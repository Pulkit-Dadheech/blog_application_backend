...
app.put("/api/update", upload.single("file"), async (req, res) => {
  try {
    ...
    if (oldPublicId) {
      ...
      if (resource) {
        ...
        if (!deleteSuccess) {
          return res
            .status(500)
            .json({ message: "Failed to delete old media" });
        }
      }
    }
    ...
    if (!newMedia)
      return res.status(500).json({ message: "Failed to upload new media" });
    ...
    return res
      .status(500)
      .json({ message: "Error updating media", error: error.message });
  }
});
